...
- **设置读写走本插件自己的路由**：`settingsScope` 不再列为必需服务（必需服务缺失会让 DSH
  报**启动级横幅**，而不只是少个卡片）；缺失时由一个**接口同形**的适配器接上
  `GET/POST /plugins/dsh-workbuddy2api/config`。卡片自身的设置代码两线完全相同。

写入只做**单字段合并**，不做整段替换 —— 线上传输从不携带标记为 secret 的字段，
按浏览器手上那份重建整段会把密钥**静默删掉**。

两条线的功能因此一致：模型调用、账号池轮换与失败换号、冷却/熔断、模型级限额、定时成长任务、
启动自动签到、以及在设置面板里改模型选择 / 改定时时间 / 浏览器登录新增账号。
