...
- **按账号积分、剩余额度环与签到** —— 卡片里逐个账号显示剩余积分、
  ...
  （就在环的右边）。算不出比值时只画灰环并标注「剩余额度未知」，
  不会编一个百分比。积分按需刷新，不随卡片轮询打上游。
- **自动成长任务** —— 卡片列出该账号在上游的全部任务（进度 / 奖励 / 可领取），
  并把**纯 API 就能点亮**的任务一键做完：报名 → 上报行为事件 →
  等上游异步计分 → 达标自动领奖。默认**每天 00:05** 跑一轮，
  并在 **DSH 启动约 20 秒后**再跑一轮，两处都能在卡片里改。
  需要官方客户端内交互的任务会**如实标注并说明原因**，不会去发一条注定不计分的请求。
...
**哪些任务不碰**：需要真实客户端交互才能计分的（召唤专家、技能加载、
用指定模型对话、夜猫子夜间窗口、真实捐款等）在卡片里标注「需客户端操作」
并给出原因 —— 宁可如实说明，也不发一条注定不计分的请求。
...
- **成长任务**：列出该账号的全部任务与进度；「一键完成可自动任务」按下面的顺序
  跑一遍；账号那一行的「做任务」只跑该账号；自动执行区可以开关「每天执行」、
  改执行时刻、开关「启动后也执行一次」，并显示下次 / 上次执行时间与上一轮的
  逐项结果。
