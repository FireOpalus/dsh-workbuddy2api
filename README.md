...
`peerDependencies` 逐项写明范围。注意 semver 的一条规则：**预发布版本只有在范围里存在
「同一 major.minor.patch 且带预发布」的比较器时才匹配**，所以 0.1.7 的元组必须显式列出
（`>=0.1.5-rc.2 <0.2.0 || >=0.1.7-rc.2 <0.2.0`）。每新增一条 DSH 预发布线，都要把该元组补进去。

### 0.1.7-rc.2 上的已知缺口

0.1.7-rc.2 移除了 `settings.plugin.item` 槽位与 `settingsScope` 服务。本插件已改为
**按宿主实际声明的槽位注册**（有 `settings.section` 就注册成设置面板里的独立页面，否则退回旧槽位），
且不再把 `settingsScope` 列为必需服务 —— 必需服务缺失会让 DSH 报启动级横幅，而不只是少个卡片。

- **设置面板里有独立页面「WorkBuddy 账号池」**，账号池、健康度、积分可见
- **不受影响**：模型调用、账号池加权轮换与失败换号、冷却/熔断、模型级限额、定时成长任务、
  启动自动签到 —— 全部在宿主侧，与卡片无关
- **暂时不可用**：改模型选择、改定时时间、浏览器登录新增账号 ——
  这些依赖已被移除的 `settingsScope`，读写在下一版改由宿主路由支撑
...
- **每次启动自动签到** —— DSH 启动后自动为全部账号签到一次（不做定时排程）。
  奖励不多，真正有用的是副作用：因积分耗尽而被打入硬冷却的账号，会由它自己的
  签到恢复余额并被**立即解冻**，而不是被闲置到次日 04:00。已签到视为幂等成功；
  网络错误则仍算失败。国际版没有签到体系，不发任何请求。
- **并发上限** —— 单账号默认 3、国际版账号 2、整池 8，全可调。
- **模型级限额与成本分层** —— 上游的 `6004` 是「**这个模型**超限」，
  因此只对该模型避让、账号继续服务其他模型（换模型立刻可用）；
  `11102`「该后端无此模型」按 `(账号, 模型)` 负缓存退避。
  选号还会按**实测扣费**（上游 `usage.credit`）分层：已实测免费的账号硬优先，
  未知的先于已知收费的（否则免费的号永远轮不到，也就永远学不到它是免费的）。
- **WAF / Retry-After / 重置墙钟** —— 网关 403（无业务信封）按账号软冷却
  （不禁用，到期自愈）；`Retry-After` / `retry-after-ms` / `x-ratelimit-reset`
  三种头都采信；body 里的「将在 … 重置」（按 UTC+8）直接作为冷却截止，
  **不做指数堆加** —— 上游说了何时恢复，等更久只会白白闲置账号。
- **统计与积分跨重启保留** —— 累计成功/失败、最近使用与最近错误、缓存积分，
  以及熔断/降权/软冷却的升级计数，都写进宿主自己的
  `$DSH_HOME/.workbuddy2api-pool-state.json`，重启或升级后继续累加，
  退避也接着算而不是从「第一次」重来。**在途数刻意不保存**：重启后没有任何
  请求在途，恢复一个永远不会被释放的在途计数会永久占用该账号的并发额度。
  `doctor` 会报告计数文件状态与逐账号统计。
...
池策略（错误分类链、模型级冷却、分层选号、WAF/Retry-After/重置墙钟、
快过期窗口、粘性键与分配算法）与 workbuddy2api 的逐项对比、
实测记录、以及「哪些已足够、哪些刻意没做」的判断，见
