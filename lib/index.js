import { A as defaultDesktopAuthDirs, B as parseCreditMultiplier, C as applyContextBudgets, D as WorkBuddyCredentialStore, E as WORKBUDDY_AUTH_FILE_ENV, F as workbuddyAccountId, G as regionOf, H as parseUpstreamModel, I as workbuddyOwnAuthPath, K as selectCliModels, L as WorkBuddyUpstreamClient, M as expiryToMs, N as isFresher, O as authFileName, P as parseWorkBuddyAuth, R as classifyUpstreamError, S as WorkBuddyCatalog, T as fallbackModelsFor, U as parseUpstreamTask, V as parseReasoning, W as prepareChatBody, _ as WorkBuddyAccountPool, a as readHostHeartbeat, b as FALLBACK_WORKBUDDY_MODELS, c as WORKBUDDY2API_VERSION, d as clearPoolState, f as parsePoolCounterRecord, g as DEFAULT_WORKBUDDY_POOL_POLICY, h as writePoolState, i as processStartTimeMs, j as defaultDesktopAuthPath, k as defaultDesktopAuthCandidates, l as WORKBUDDY2API_POOL_STATE_FILENAME, m as workbuddyPoolStatePath, n as clearHostHeartbeat, o as workbuddyHostHeartbeatPath, p as readPoolState, r as isHeartbeatProcessAlive, s as writeHostHeartbeat, t as WORKBUDDY2API_HOST_HEARTBEAT_FILENAME, u as WORKBUDDY2API_POOL_STATE_VERSION, v as nextDay4Am, w as deriveCatalog, x as FALLBACK_WORKBUDDY_MODELS_GLOBAL, y as stickyKeyOf, z as isNightWindow } from "./host-heartbeat-V03g_S8b.js";
...
function usageOf(frame) {
	...
	if (credit === null || tokens === null) return void 0;
	return {
		credit: Number(credit[1]),
		totalTokens: Number(tokens[1])
	};
}
	...
	async function chatCompletions(req, res) {
		...
		for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
			...
			if (result.ok) {
				...
				body.on("data", (chunk) => {
					if (chunk.includes("[DONE]")) {
						...
						if (usage !== void 0 && model !== void 0) pool.noteModelCost(accountId, model, usage.credit, usage.totalTokens);
					}
			...
			pool.report(accountId, {
				...
				...model === void 0 ? {} : { model },
				...result.resetAtMs === void 0 ? {} : { resetAtMs: result.resetAtMs },
...
*/
const WORKBUDDY2API_CONFIG_PATH = "/plugins/dsh-workbuddy2api/config";
		...
		const disposeConfig = ctx.webServer.register({
			...
			handler: async (req, res) => {
				...
				if (read === void 0) {
					json(res, 501, {
						writable: false,
						error: "this host has no configuration route"
					});
					return;
				}
				if (req.method === "GET") {
				...
				if (write === void 0) {
					json(res, 403, {
						writable: false,
						error: "this deployment stores settings read-only"
					});
					return;
				}
				try {
					const body = await readJsonBody(req);
					const field = typeof body?.["field"] === "string" ? body["field"] : void 0;
...
*/
const ALREADY_MARKERS = ["已签到", "already"];
...
*/
function isAlreadyCheckedIn(error) {
...
*/
async function checkinAllAccounts(options) {
	const results = [];
	let accounts = [];
	try {
	...
	for (const account of accounts) {
		...
		try {
			...
			if (!status.active) {
				...
				outcome = "already";
				message = "今日已签到";
			} else credit = (await options.client.claimDailyCheckin(credential)).credit;
		} catch (error) {
	...
	options.log("checkin at startup: " + String(claimed) + " 签到成功" + (already === 0 ? "" : "，" + String(already) + " 今日已签到") + (failed === 0 ? "" : "，" + String(failed) + " 失败") + (unfrozen === 0 ? "" : "，解冻 " + String(unfrozen) + " 个账号") + "，+ " + String(credit) + " 积分");
	for (const result of results) if (result.outcome === "error") options.log("  " + result.accountName + ": " + result.message);
	return results;
}
//#endregion
//#region src/index.ts
...
*/
const WORKBUDDY2API_ENTRY_ID = "dsh-workbuddy2api";
...
const Config = z.object({
	...
	regions: volatileField(z.dict(regionStateConfig).default({}).description("Per-region model directory, selection, and pool state, keyed cn | global")),
...
*/
const COUNTER_PERSIST_DEBOUNCE_MS = 2e3;
...
function apply(ctx, config) {
	...
	*/
	...
	ctx.inject(["webServer"], (webCtx) => registerWorkBuddy2ApiStatusRoute(webCtx, {
		...
		...typeof settingsService.update !== "function" ? {} : { writeConfigField: async (field, value) => {
	...
	if (typeof settingsService.configure === "function") {
		...
		if (typeof describe === "function") current = () => {
			for (const id of [WORKBUDDY2API_ENTRY_ID, WORKBUDDY2API_SETTINGS_NS]) {
				...
				if (descriptor?.value !== void 0) return descriptor.value;
			}
			return config;
		};
		ctx.inject(["settings"], (child) => {
			const service = child.settings;
			child.effect(() => service.configure?.({ auto: false }, ctx.fiber) ?? (() => {}), "dsh-workbuddy2api: settings page policy");
		});
	} else if (typeof settingsService.installSection === "function") settingsService.installSection(ctx, WORKBUDDY2API_SETTINGS_NS, Config, config, {
		setSource(source) {
			current = source;
		},
		onChange() {
	...
	Promise.all(REGION_KEYS.map((region) => stacks[region].shim.ready)).then(async () => {
		...
		(async () => {
			...
			try {
				...
				for (const region of REGION_KEYS) {
					const records = document.regions[region] ?? [];
					if (records.length > 0) stacks[region].pool.restoreCounters(records);
				}
				for (const region of REGION_KEYS) counters.regions[region] = [...stacks[region].pool.toCounters().values()];
				countersLoaded = true;
