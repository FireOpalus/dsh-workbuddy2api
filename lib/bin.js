#!/usr/bin/env node
import { A as defaultDesktopAuthDirs, D as WorkBuddyCredentialStore, E as WORKBUDDY_AUTH_FILE_ENV, L as WorkBuddyUpstreamClient, T as fallbackModelsFor, _ as WorkBuddyAccountPool, a as readHostHeartbeat, c as WORKBUDDY2API_VERSION, d as clearPoolState, g as DEFAULT_WORKBUDDY_POOL_POLICY, k as defaultDesktopAuthCandidates, m as workbuddyPoolStatePath, o as workbuddyHostHeartbeatPath, p as readPoolState, r as isHeartbeatProcessAlive } from "./host-heartbeat-C8h5QNsg.js";
	...
	const counterSummary = () => {
		...
		return {
			...
			accounts: records.map((record) => ({
				...
				...record.credits === void 0 ? {} : { credits: record.credits },
				...record.creditsAtMs === void 0 ? {} : { creditsAt: new Date(record.creditsAtMs).toISOString() }
			}))
		};
	};
	...
	if (jsonOutput) printJson({
		...
		...fragments.flatMap((fragment) => [
			...
			...fragment.entries.flatMap((entry) => [
				...
				...entry.modelCooldowns === void 0 || entry.modelCooldowns.length === 0 ? [] : [`    model limits: ${entry.modelCooldowns.map((cooldown) => `${cooldown.model} until ${new Date(cooldown.untilMs).toISOString()} (${cooldown.reason})`).join(", ")}`],
