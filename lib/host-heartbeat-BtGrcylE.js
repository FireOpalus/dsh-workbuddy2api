...
function isModelBlocked(status, body) {
	...
	if (typeof inner === "object" && inner !== null) nodes.push(inner);
	let code = "";
	let message = "";
	for (const node of nodes) {
		for (const key of [
			...
			if (value !== void 0 && value !== null && code === "") code = String(value).trim();
		}
...
const RESET_ZONE_OFFSET_MINUTES = 480;
...
function parseRateReset(body) {
	const captured = RESET_PATTERN.exec(body)?.[1];
	...
	const utc = Date.UTC(Number(year), Number(month) - 1, Number(day), Number(hour), Number(minute), Number(second)) - RESET_ZONE_OFFSET_MINUTES * 6e4;
	return Number.isFinite(utc) ? utc : void 0;
}
...
const RETRY_AFTER_CEILING_MS = 72e5;
/**
...
function parseRetryAfter(headers) {
	for (const name of RETRY_AFTER_HEADERS) {
		...
		const milliseconds = name === "retry-after-ms" ? number : name === "retry-after" ? number * 1e3 : (value.length >= 12 ? number : number * 1e3) - Date.now();
...
var WorkBuddyUpstreamClient = class {
	...
	*/
	async fetchCredits(credential, expiringSoonMs) {
		...
		let nearestExpiryMs;
		let expiringSoon = 0;
		const SOON_MS = expiringSoonMs !== void 0 && expiringSoonMs > 0 ? expiringSoonMs : 2592e5;
		const parseDate = (raw) => {
...
var WorkBuddyAccountPool = class {
	...
	blankEntry(accountId) {
		return {
			...
			creditsAtMs: 0,
			sessionDeadFails: 0,
			modelCooldowns: /* @__PURE__ */ new Map(),
	...
	*/
	...
	view(entry, now) {
		return {
			...
			...(() => {
				const active = [...entry.modelCooldowns].filter(([, cooldown]) => cooldown.untilMs > now).map(([model, cooldown]) => ({
					model,
	...
	*/
	healthyForModel(entry, now, model) {
		if (!this.healthy(entry, now)) return false;
		if (model === void 0 || model === "") return true;
		const blocked = entry.modelCooldowns.get(model);
		return blocked === void 0 || blocked.untilMs <= now;
	}
	/** Drop per-model cooldowns that have expired, so the map cannot grow forever. */
	pruneModelCooldowns(entry, now) {
		if (entry.modelCooldowns.size === 0) return;
		for (const [model, cooldown] of entry.modelCooldowns) if (cooldown.untilMs <= now) entry.modelCooldowns.delete(model);
	}
	...
	*/
	pick(options = {}) {
		...
		if (bound !== void 0 && this.healthyForModel(bound, now, model) && !excluded.has(bound.accountId) && this.totalInFlight < this.policy.maxInFlightTotal) return {
			ok: true,
		...
		let candidates = all.filter((entry) => !excluded.has(entry.accountId) && this.healthyForModel(entry, now, model));
		if (options.stickyKey !== void 0 && candidates.length > 1) {
		...
		const tierOf = (entry) => {
			...
			if (observed === void 0 || now - observed.lastSeenMs > MODEL_COST_TTL_MS) return {
				tier: 1,
				cost: 0
			};
			return {
				tier: observed.costPer1k <= 0 ? 0 : 2,
				cost: observed.costPer1k
			};
		};
		const tiered = candidates.map((entry) => ({
			entry,
			...tierOf(entry)
		}));
		...
		for (const candidate of inTier) {
			...
			if (credits !== void 0 && credits > maxCredits) maxCredits = credits;
		}
		const ranked = inTier.map((candidate) => ({
			entry: candidate.entry,
			weight: this.weightOf(candidate.entry, maxCredits, now),
			cost: candidate.cost
		})).sort((left, right) => left.cost - right.cost || right.weight - left.weight || left.entry.usedSeq - right.entry.usedSeq);
		...
		const chosen = eligible.length > 0 ? this.pickWeighted(eligible.map((candidate) => candidate.entry), eligible.map((candidate) => candidate.weight)) : fullRanked.reduce((best, candidate) => candidate.entry.usedSeq < best.entry.usedSeq ? candidate : best).entry;
		return {
			ok: true,
	...
	*/
	report(accountId, outcome, stickyKey) {
		...
		if (outcome.ok) {
			...
			entry.lastError = "";
			if (outcome.model !== void 0 && outcome.model !== "") {
				const cached = entry.modelCooldowns.get(outcome.model);
				if (cached !== void 0 && cached.reason.startsWith("11102")) entry.modelCooldowns.delete(outcome.model);
			}
		...
		switch (outcome.kind) {
			case "hard_credit":
				...
				return;
			case "model_rate": {
				const model = outcome.model !== void 0 && outcome.model !== "" ? outcome.model : "";
				if (model === "") {
					this.applySoftRate(entry, now, outcome);
					return;
				}
				const ceiling = this.policy.softRateCooldownMaxMs;
				const until = outcome.resetAtMs !== void 0 ? Math.min(outcome.resetAtMs, now + ceiling) : now + Math.min(this.policy.softRateCooldownMs, ceiling);
				const previous = entry.modelCooldowns.get(model);
				entry.modelCooldowns.set(model, {
					untilMs: until,
					reason: outcome.resetAtMs !== void 0 ? "6004 model rate limit" : "model rate limited",
			...
			case "model_blocked": {
				const model = outcome.model !== void 0 && outcome.model !== "" ? outcome.model : "";
				if (model === "") return;
				const hits = (entry.modelCooldowns.get(model)?.hits ?? 0) + 1;
	...
	* refusal did.
	*/
	applySoftRate(entry, now, outcome, baseOverrideMs) {
		const base = baseOverrideMs ?? this.policy.softRateCooldownMs;
		if (outcome.resetAtMs !== void 0) {
			entry.cooldownKind = "soft";
		...
		const grown = base * 2 ** Math.min(entry.softStreak, 16);
		entry.cooldownKind = "soft";
	...
	*/
	noteModelCost(accountId, model, credit, totalTokens) {
		if (model === "" || !Number.isFinite(credit) || credit < 0 || !Number.isFinite(totalTokens) || totalTokens <= 0) return;
		const entry = this.entries.get(accountId);
		if (entry === void 0) return;
		entry.modelCost.set(model, {
			costPer1k: credit / totalTokens * 1e3,
			lastSeenMs: this.now()
		});
		const now = this.now();
		for (const [name, observation] of entry.modelCost) if (now - observation.lastSeenMs > MODEL_COST_TTL_MS * 4) entry.modelCost.delete(name);
	}
	...
	*/
	reenableIfCredits(accountId, credits) {
		const entry = this.entries.get(accountId);
		if (entry === void 0) return false;
		entry.credits = credits.total;
		if (credits.expiringSoon !== void 0) entry.creditsExpiring = credits.expiringSoon;
		if (credits.capacity !== void 0) entry.creditsCapacity = credits.capacity;
		entry.creditsAtMs = this.now();
		if (credits.total <= 0 || !entry.enabled) return false;
		const unfrozen = entry.cooldownUntil > 0 || entry.cooldownKind !== "none" || entry.softStreak > 0 || entry.degradedUntil > 0 || entry.modelCooldowns.size > 0;
		entry.cooldownUntil = 0;
		entry.cooldownKind = "none";
		entry.softStreak = 0;
		entry.degradedUntil = 0;
	...
	modelCooldownsOf(accountId) {
		const entry = this.entries.get(accountId);
		if (entry === void 0) return [];
		const now = this.now();
		return [...entry.modelCooldowns].filter(([, cooldown]) => cooldown.untilMs > now).map(([model, cooldown]) => ({
			model,
			untilMs: cooldown.untilMs,
			reason: cooldown.reason,
			hits: cooldown.hits
	...
	*/
	toCounters() {
		...
		for (const entry of this.entries.values()) out.set(entry.accountId, {
			accountId: entry.accountId,
			successes: entry.successes,
			failures: entry.failures,
	...
	*/
	restoreCounters(records) {
		for (const record of records) {
			const entry = this.entries.get(record.accountId);
			if (entry === void 0) continue;
			if (record.successes !== void 0) entry.successes = record.successes;
			if (record.failures !== void 0) entry.failures = record.failures;
			if (record.usedSeq !== void 0) entry.usedSeq = record.usedSeq;
			if (record.lastUsedAt !== void 0) entry.lastUsedAt = record.lastUsedAt;
			if (record.lastSuccessAt !== void 0) entry.lastSuccessAt = record.lastSuccessAt;
			if (record.lastErrorAt !== void 0) entry.lastErrorAt = record.lastErrorAt;
			if (record.lastError !== void 0) entry.lastError = record.lastError;
			if (record.credits !== void 0) entry.credits = record.credits;
			if (record.creditsExpiringSoon !== void 0) entry.creditsExpiring = record.creditsExpiringSoon;
			if (record.creditsCapacity !== void 0) entry.creditsCapacity = record.creditsCapacity;
			if (record.creditsAtMs !== void 0) entry.creditsAtMs = record.creditsAtMs;
			if (record.softStreak !== void 0) entry.softStreak = record.softStreak;
			if (record.breakerFails !== void 0) entry.breakerFails = record.breakerFails;
			if (record.breakerTrips !== void 0) entry.breakerTrips = record.breakerTrips;
			if (record.consecutiveFails !== void 0) entry.consecutiveFails = record.consecutiveFails;
		}
		for (const entry of this.entries.values()) if (entry.usedSeq > this.seq) this.seq = entry.usedSeq;
	}
...
const WORKBUDDY2API_POOL_STATE_FILENAME = ".workbuddy2api-pool-state.json";
/** Current on-disk format; readers reject others. */
const WORKBUDDY2API_POOL_STATE_VERSION = 1;
/** Absolute path of the counter file. */
function workbuddyPoolStatePath(storeDir = resolveDshHome()) {
...
function parsePoolCounterRecord(value) {
	if (typeof value !== "object" || value === null || Array.isArray(value)) return void 0;
	const raw = value;
	...
	return {
		accountId,
		...finiteNumber(raw["successes"]) === void 0 ? {} : { successes: finiteNumber(raw["successes"]) },
		...finiteNumber(raw["failures"]) === void 0 ? {} : { failures: finiteNumber(raw["failures"]) },
		...finiteNumber(raw["usedSeq"]) === void 0 ? {} : { usedSeq: finiteNumber(raw["usedSeq"]) },
		...finiteNumber(raw["lastUsedAt"]) === void 0 ? {} : { lastUsedAt: finiteNumber(raw["lastUsedAt"]) },
		...finiteNumber(raw["lastSuccessAt"]) === void 0 ? {} : { lastSuccessAt: finiteNumber(raw["lastSuccessAt"]) },
		...finiteNumber(raw["lastErrorAt"]) === void 0 ? {} : { lastErrorAt: finiteNumber(raw["lastErrorAt"]) },
		...lastError === void 0 ? {} : { lastError },
		...finiteNumber(raw["credits"]) === void 0 ? {} : { credits: finiteNumber(raw["credits"]) },
		...finiteNumber(raw["creditsExpiringSoon"]) === void 0 ? {} : { creditsExpiringSoon: finiteNumber(raw["creditsExpiringSoon"]) },
		...finiteNumber(raw["creditsCapacity"]) === void 0 ? {} : { creditsCapacity: finiteNumber(raw["creditsCapacity"]) },
		...finiteNumber(raw["creditsAtMs"]) === void 0 ? {} : { creditsAtMs: finiteNumber(raw["creditsAtMs"]) },
		...finiteNumber(raw["softStreak"]) === void 0 ? {} : { softStreak: finiteNumber(raw["softStreak"]) },
		...finiteNumber(raw["breakerFails"]) === void 0 ? {} : { breakerFails: finiteNumber(raw["breakerFails"]) },
		...finiteNumber(raw["breakerTrips"]) === void 0 ? {} : { breakerTrips: finiteNumber(raw["breakerTrips"]) },
		...finiteNumber(raw["consecutiveFails"]) === void 0 ? {} : { consecutiveFails: finiteNumber(raw["consecutiveFails"]) }
	};
}
/** Read the counter document; absent, unreadable, or malformed reads as empty. */
async function readPoolState(storeDir = resolveDshHome()) {
	const empty = {
		version: 1,
		regions: {}
	};
	...
	const regions = typeof document["regions"] === "object" && document["regions"] !== null ? document["regions"] : {};
	const out = {
		version: 1,
		regions: {}
	};
...
async function writePoolState(document, storeDir = resolveDshHome()) {
...
async function clearPoolState(storeDir = resolveDshHome()) {
...
const WORKBUDDY2API_VERSION = "0.6.5";
//#endregion
...
//#endregion
export { defaultDesktopAuthDirs as A, parseCreditMultiplier as B, applyContextBudgets as C, WorkBuddyCredentialStore as D, WORKBUDDY_AUTH_FILE_ENV as E, workbuddyAccountId as F, regionOf as G, parseUpstreamModel as H, workbuddyOwnAuthPath as I, selectCliModels as K, WorkBuddyUpstreamClient as L, expiryToMs as M, isFresher as N, authFileName as O, parseWorkBuddyAuth as P, classifyUpstreamError as R, WorkBuddyCatalog as S, fallbackModelsFor as T, parseUpstreamTask as U, parseReasoning as V, prepareChatBody as W, WorkBuddyAccountPool as _, readHostHeartbeat as a, FALLBACK_WORKBUDDY_MODELS as b, WORKBUDDY2API_VERSION as c, clearPoolState as d, parsePoolCounterRecord as f, DEFAULT_WORKBUDDY_POOL_POLICY as g, writePoolState as h, processStartTimeMs as i, defaultDesktopAuthPath as j, defaultDesktopAuthCandidates as k, WORKBUDDY2API_POOL_STATE_FILENAME as l, workbuddyPoolStatePath as m, clearHostHeartbeat as n, workbuddyHostHeartbeatPath as o, readPoolState as p, isHeartbeatProcessAlive as r, writeHostHeartbeat as s, WORKBUDDY2API_HOST_HEARTBEAT_FILENAME as t, WORKBUDDY2API_POOL_STATE_VERSION as u, nextDay4Am as v, deriveCatalog as w, FALLBACK_WORKBUDDY_MODELS_GLOBAL as x, stickyKeyOf as y, isNightWindow as z };
