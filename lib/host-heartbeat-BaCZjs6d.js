	...
	*/
	async fetchModelsForCredentials(credentials, signal) {
		...
		for (const result of settled) {
			...
			for (const model of result.value) if (byId.get(model.id) === void 0) byId.set(model.id, model);
		}
		if (byId.size === 0) {
...
*/
const FALLBACK_WORKBUDDY_MODELS_UNION = (() => {
	const byId = /* @__PURE__ */ new Map();
	for (const model of [...FALLBACK_WORKBUDDY_MODELS, ...FALLBACK_WORKBUDDY_MODELS_GLOBAL]) if (!byId.has(model.id)) byId.set(model.id, model);
	return [...byId.values()];
})();
...
var WorkBuddyCatalog = class {
	models;
	constructor(seed = FALLBACK_WORKBUDDY_MODELS_UNION) {
		this.models = seed;
	}
...
const WORKBUDDY2API_VERSION = "0.1.1";
//#endregion
...
//#endregion
export { WorkBuddyUpstreamClient as A, defaultDesktopAuthDirs as C, parseWorkBuddyAuth as D, isFresher as E, prepareChatBody as F, regionOf as I, selectCliModels as L, parseCreditMultiplier as M, parseReasoning as N, workbuddyAccountId as O, parseUpstreamModel as P, defaultDesktopAuthCandidates as S, expiryToMs as T, applyContextBudgets as _, readHostHeartbeat as a, WorkBuddyCredentialStore as b, WORKBUDDY2API_VERSION as c, nextDay4Am as d, stickyKeyOf as f, WorkBuddyCatalog as g, FALLBACK_WORKBUDDY_MODELS_UNION as h, processStartTimeMs as i, classifyUpstreamError as j, workbuddyOwnAuthPath as k, DEFAULT_WORKBUDDY_POOL_POLICY as l, FALLBACK_WORKBUDDY_MODELS_GLOBAL as m, clearHostHeartbeat as n, workbuddyHostHeartbeatPath as o, FALLBACK_WORKBUDDY_MODELS as p, isHeartbeatProcessAlive as r, writeHostHeartbeat as s, WORKBUDDY2API_HOST_HEARTBEAT_FILENAME as t, WorkBuddyAccountPool as u, deriveCatalog as v, defaultDesktopAuthPath as w, authFileName as x, WORKBUDDY_AUTH_FILE_ENV as y };
