		...
		*/
		...
		function apply(ctx) {
			try {
				...
				const settingsScope = ctx.settingsScope === void 0 ? createRouteSettingsScope({ url: WORKBUDDY2API_CONFIG_PATH }) : ctx.settingsScope.bind({ namespace: "workbuddy2api" });
				const injected = () => ({
					t,
					settingsScope
				});
				...
				let mounted = false;
				const declared = (key) => typeof slots.specDynamic === "function" && slots.specDynamic(key) !== void 0;
				...
				const mount = () => {
					...
					console.info("[dsh-workbuddy2api] settings card mounted into " + (section ? "settings.section" : "settings.plugin.item"));
					return section ? registerSection() : registerItem();
				};
				const arm = (key) => {
					if (mounted || declared(key)) {
						mount();
						return;
					}
